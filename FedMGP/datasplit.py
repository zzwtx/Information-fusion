...
def record_net_data_stats(y_train, net_dataidx_map, logdir=None):
    ...
    if logdir != None:
        client_count = len(net_cls_counts)
        total_classes = set()
        for client_stats in net_cls_counts.values():
            total_classes.update(client_stats.keys())
        logger.info(f'Data distribution: {client_count} clients, {len(total_classes)} classes')
