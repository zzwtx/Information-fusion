...
class MaPLe(TrainerX):
    ...
    def forward_backward(self, batch, global_weight=None, fedprox=False, mu=0.5):
        ...
        if prec == "amp":
            ...
            loss = model(image, label)
            if fedprox and global_weight is not None:
                # FedProx regularization
                model_weight = self.model.state_dict()
                fed_prox_reg = 0
                for key in global_weight.keys():
                    if key in model_weight:
                        fed_prox_reg += ((mu / 2) * torch.norm((model_weight[key] - global_weight[key])) ** 2)
                loss += fed_prox_reg
            optim.zero_grad()
            loss.backward()
            optim.step()

        # Compute accuracy separately since MaPLe returns loss during training
        with torch.no_grad():
            self.model.eval()
